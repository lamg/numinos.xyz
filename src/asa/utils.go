...
func writeResp(resp ...*assetResp) {
	...
	if err != nil {
		bs = []byte(fmt.Sprintf(`{"err": "failed JSON serialization %v with %s"}`, resp, err))
	}
	fmt.Println(string(bs))
}

func writeMnemonic(resp ...*addressMnemonic) {
	for _, j := range resp {
		if j.err != nil {
			j.Err = j.err.Error()
		}
...
func writeAccounts(resp ...*accountResp) {
	for _, j := range resp {
		if j.err != nil {
			j.Err = j.err.Error()
		}
